...
CREATE TABLE products (
    id text primary key, -- Usiamo ID testuali come 'caffe', 'cappuccino' per semplicità
    name text not null,
...
INSERT INTO products (id, name, price) VALUES
    ('caffe', '☕ Caffè', 1.2),
    ('ginseng_amaro', '☕ Ginseng Amaro', 1.5),
    ('zucchero_di_canna', '☕ Zucchero di Canna', 1.5),
    ('cappuccino', '🥛 Cappuccino', 1.5),
    ('cornetto_crema_limone', '🥐 Cornetto Crema Limone', 1.3),
    ('succo', '🧃 Succo di frutta', 1.2),
    ('te_limone', '🍵 Tè al Limone', 1.4),
    ('te_pesca', '🍵 Tè alla Pesca', 1.4);

-- 5. Abilita l'accesso in lettura (SELECT) per tutti (anon)
-- Supabase usa Row Level Security (RLS). Per semplicità, abilitiamo la lettura pubblica.
